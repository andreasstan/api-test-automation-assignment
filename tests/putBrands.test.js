...
const nonExistentBrandId = "01XXXXXXXXXXXXXX"; // ID acak yang tidak ada di database
...
test("Should update a brand successfully with valid data", async () => {
    try {
        ...
        throw error; // Re-throw the error so Jest still marks the test as failed
    }
});

// Test Case 2: Attempt to update a non-existent brand (404)
test("Should return 404 when updating a non-exist brand", async () => {
    try {
        const response = await request(baseURL)
    ...
    try {
        const response = await request(baseURL)
            ...
            .send({
                ...
                slug: true // Invalid slug (should be a string)
            });
...
test("Should return 200 when updating with only 1 parameter", async () => {
    try {
        const response = await request(baseURL)
            ...
            .set("Content-Type", "application/json")
            .send({
                name: randomName, // Missing slug field
            });
...
// Test Case 5: Update with Invalid Data Type
test("Should return 422 when updating brand with invalid data types", async () => {
    try {
        const response = await request(baseURL)
            .put(`/brands/${validBrandId}`)
            .set("Accept", "application/json")
...
// Test Case 6: Update with null values for name and slug
...
// Test Case 7: Update with brand and slug containing spaces
...
// Test Case 8: Update with special characters in name and slug
